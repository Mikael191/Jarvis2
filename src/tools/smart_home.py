...
async def control_smart_home(
    device_name: str, action: str, parameters: dict | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        domain = (
            "light"
            if "luz" in device_name.lower() or "light" in device_name.lower()
            else "switch"
        )
        ...
        return {
            "success": True,
            "output": f"Comando enviado com sucesso para {device_name}.",
            "error": "",
        }
        ...
        return {
            "success": False,
            "output": "",
            "error": f"Erro de conexão com o Smart Home: {exc}",
        }
